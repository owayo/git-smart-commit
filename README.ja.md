...
- それ以外のモードでは、`--quiet` と併用した場合も含め、デバッグ出力は一体で標準出力に出ます。`--quiet` は進捗メッセージを抑制するだけで、デバッグ出力の出力先は変えません
...
既存のグローバル設定ファイルは自動では書き換えられません。現在の Codex 既定モデルは `gpt-5.6-luna` です。既存設定で使うには、`~/.config/git-sc/config.toml` の `models.codex` を更新してください。**Codex CLI を更新したら必ず確認してください。** 以前の既定値 `gpt-5.6-luna` は Codex から削除済みで、削除されたモデル名を指定してもフォールバックは起きません。HTTP 400 が返り、git-sc はこれをプロバイダーの失敗として扱ってクールダウンに入れるため、古い `models.codex` が残っていると毎回 Codex がフォールバックチェーンから静かに外れます。この既定値は、API で利用可能・一覧表示対象・`medium` reasoning 対応の Codex モデルについて `input_tokens` を比較し、2026年9月17日 (JST) に再選定したものです。計測は空ディレクトリで `Reply ok.` を使い、`--ignore-user-config --ignore-rules --ephemeral --sandbox read-only` と `model_reasoning_effort='medium'` を指定しました: `gpt-5.6-luna` = 19609、`gpt-5.5` = 20181、`gpt-5.6-sol` = 21174、`gpt-5.6-terra` = 21174、`gpt-6-astra` = 22035。いずれの試行も最終出力が `ok` でツール呼び出しはなく、2 回目の計測でも全モデルが同一値を再現しました。
...
一致したルールの `prefix_type` は上記の有効値である必要があります。無効な一致ルールは警告を出してスキップされるため、後続のプレフィックスルール、設定済みの `prefix_type`、または自動判定にフォールバックできます。
...
- **`.git-sc-ignore` は diff の表示形式に関する Git 設定の影響を受けません。** 除外判定は `diff --git a/… b/…` 行からファイルパスを読んで行うため、この行の形を変える設定 (`diff.noprefix` / `diff.mnemonicPrefix` / `diff.srcPrefix` / `diff.dstPrefix` / `color.ui = always` / `diff.external`) があると、パターンが黙って一切マッチしなくなります。git-sc は diff 取得時にプレフィックス・色・パスの基準を固定して要求するので、これらの設定に関係なく同じようにパターンが適用されます。`diff.relative` も同様です。この設定は加えて「git-sc を実行したディレクトリの外にある変更を diff から隠す」効果を持ちますが、基準を固定しているため、どのサブディレクトリで実行してもステージ済みの差分全体からメッセージが書かれます。
...
`ai_usage_group` があるのは、1 つのアカウントの枠がモデル系統ごとに分かれている場合があるためです。Antigravity は `Gemini` と `Claude&GPT` という別々の枠を報告し、それぞれ独立に消費されます。`ai_usage_profile` を指定しない場合、git-sc はそのプロバイダーで最も使用率の低いアカウントを基準に判定します。ただしこれは **判定にしか影響しません**。ステップが実際にどのアカウントで動くかは `env` が決めるため、判定と実行を一致させたい場合は両方を指定してください。
