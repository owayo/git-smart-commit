...
- The default Codex model is `gpt-5.2` (`default_codex_model()` in `config.rs`). It was re-validated on May 31, 2026 (JST) against the current `codex debug models` list (`gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex`, `gpt-5.3-codex-spark`, `gpt-5.2`, `codex-auto-review`). Single-run `tokens used` from `echo "Hello" | codex exec` turned out to be unreliable for model selection: the same model/input varied by up to ~50x across runs because of prompt caching, and the measurement used `model_reasoning_effort=medium` whereas git-sc actually invokes Codex with `low` (the `codex_reasoning_effort` default), so the numbers are not representative of the real workload. Because no model showed a robust, representative advantage, `gpt-5.2` is kept as the default. Any future change should be backed by repeated measurements on a representative diff with caching effects separated, not single-run `Hello` token counts.
