...
- The default Codex model is `gpt-5.4` (`default_codex_model()` in `config.rs`). It was selected on June 3, 2026 (JST): the previous default `gpt-5.2` was retired from the `codex debug models` list, so the candidates were narrowed to models that are API-visible, listed, and support `medium` reasoning — `gpt-5.5`, `gpt-5.4`, and `gpt-5.4-mini`. Each was measured with the fixed prompt `Reply ok.` in an empty directory (`-C <tmp> --skip-git-repo-check --ignore-user-config --ignore-rules --sandbox read-only`), reading `input_tokens` from the `--json` `turn.completed` event: `gpt-5.5` = 17138, `gpt-5.4` = 15750, `gpt-5.4-mini` = 15401. `gpt-5.4-mini` is the lowest, but only ~2% (349 tokens) below `gpt-5.4`; because git-sc invokes Codex with `low` reasoning (the `codex_reasoning_effort` default) and commit-message quality matters more than a marginal token saving, `gpt-5.4` is chosen as the default, while `gpt-5.4-mini` remains available for cost-sensitive users. This measures minimal response cost only and does not guarantee real-workload (commit/review/refactor) quality; any future change should re-run `codex debug models` and re-measure, since model availability shifts over time.
